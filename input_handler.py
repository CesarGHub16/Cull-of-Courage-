...
class EventHandle(tcod.event.EventDispatch[Action]):
    ...
    def ev_mouse_pos(self, event: tcod.event.MouseMotion) -> None:
        if self.engine.game_map.in_bounds(event.tile.x, event.tile.y):
            self.engine.mouse_pos = event.tile.x, event.tile.y

    def on_render(self, console: tcod.Console) -> None:
        self.engine.render(console)


class MainGameEventHandler(EventHandle):
    def handle_events(self, context: tcod.context.Context) -> None:
        for event in tcod.event.wait():
            context.convert_event(event)
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        ...
        if key in MOVE_KEYS:
            ...
            self.engine.event_handle = HistoryViewer(self.engine)

        return None


class GameOverHandler(EventHandle):
    def handle_events(self, context: tcod.context.Context) -> None:
        for event in tcod.event.wait():
            action = self.dispatch(event)
...
class HistoryViewer(EventHandle):
