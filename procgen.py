...
class RegularRoom:
    ...
    def center(self) -> Tuple[int, int]:
        """Calculate and return the center of the room."""
...
def hallway(start: Tuple[int, int], end: Tuple[int, int]) -> Iterator[Tuple[int, int]]:
...
def place_entities(room: RegularRoom, dungeon: GameMap, max_enemy: int) -> None:
    ...
    for i in range(num_of_enemy):
        ...
        if not any(entity.x == x and entity.y == y for entity in dungeon.entities):
            if random.random() < 0.8:
                entity_factory.swordman.spawn(dungeon, x, y)
            else:
                entity_factory.ranger.spawn(dungeon, x, y)
...
def generate_dungeon(
    ...
    # Seed for consistent dungeon layout

    for r in range(max_rooms):
        room_width = random.randint(room_min_size, room_max_size)
        room_height = random.randint(room_min_size, room_max_size)

        x = random.randint(3, dungeon.width - room_width - 12)
        y = random.randint(3, dungeon.height - room_height - 12)
        ...
        if any(new_room.intersects(other_room) for other_room in rooms):
            continue  # Skip this room if it intersects
